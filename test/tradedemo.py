...
class PyCTP_Trader_API(TraderApi):
    TIMEOUT = 30
    ...
    def Login(self, BrokerID, UserID, Password):
        ...
        if ret == 0:
            ...
            if self.__rsp_Login['event'].wait(self.TIMEOUT):
                ...
                # else:
                    # sys.stderr.write(str(self.__rsp_Login['ErrorMsg'], encoding='GBK'))
    ...
    def QryInstrumentCommissionRate(self, InstrumentID):
        """ 请求查询合约手续费率 """
        QryInstrumentCommissionRate = dict(BrokerID=self.__BrokerID, InvestorID=self.__InvestorID,
                                           InstrumentID=InstrumentID)
        self.__rsp_QryInstrumentCommissionRate = dict(results=[]
                                                      , RequestID=self.__IncRequestID()
                                                      , ErrorID=0
                                                      , event=threading.Event())
        ret = self.ReqQryInstrumentCommissionRate(QryInstrumentCommissionRate,
                                                  self.__rsp_QryInstrumentCommissionRate['RequestID'])
        if ret == 0:
    ...
    def QryInvestorPosition(self, InstrumentID=b''):
        """ 请求查询投资者持仓 """
        QryInvestorPositionFiel = dict(BrokerID=self.__BrokerID, InvestorID=self.__InvestorID,
                                       InstrumentID=InstrumentID)
        self.__rsp_QryInvestorPosition = dict(results=[], RequestID=self.__IncRequestID(), ErrorID=0,
                                              event=threading.Event())
    ...
    def QryTradingAccount(self):
        ...
        self.__rsp_QryTradingAccount = dict(results=[], RequestID=self.__IncRequestID(), ErrorID=0,
                                            event=threading.Event())
    ...
    def QryDepthMarketData(self, InstrumentID):
        ...
        self.__rsp_QryDepthMarketData = dict(results=[], RequestID=self.__IncRequestID(), ErrorID=0,
                                             event=threading.Event())
    ...
    #                 sys.stderr.write(str(self.__rsp_OrderInsert['ErrorMsg'], encoding='GBK'))
    ...
    def OnErrRtnOrderAction(self, OrderAction, RspInfo):
        ...
        #    self.__rsp_OrderInsert['ErrorMsg'] = bytes('CTP:发送报单操作失败', 'GBK')
        #    self.__rsp_OrderInsert['event'].set()
...
def __main__():
    ...
    ExchangeID = 'SHFE'
    InstrumentID = 'cu1610'
    trader = PyCTP_Trader.Create('_tmp_t_')
    print('连接前置', trader.Connect('tcp://180.168.146.187:10000'))
    print('账号登陆', trader.Login(BrokerID, InvestorID, Password))
    print('投资者代码', trader.setInvestorID(InvestorID))
